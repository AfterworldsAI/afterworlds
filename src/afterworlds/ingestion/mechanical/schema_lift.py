...
SCHEMA_LIFTS: dict[tuple[str, str], SchemaLift] = {
    ...
    (SCHEMA_10_VERSION, SCHEMA_10_HASH): SchemaLift(
        ...
        rationale=(
            ...
            "change a Speed through SpeedModificationFact and state a "
            "creature's own through CreatureSpeedFact, and schema 6 added "
            "MovementAllowanceFact, which measures a granted budget by one; "
            "none of them defines what a Speed is, when it is available or "
            "how it is spent, which is the gap this closes. "
            ...
            "11 adds no field, no ownership form, no nullable field and no "
            "required field to a family an earlier schema already had. It "
            "does add two intrinsic invariants, "
            "movement_depletion.until.at-least-one and "
            "movement_depletion.until.no-repeats; both constrain "
            "movement_depletion, a family this schema mints, so neither "
            "reaches an accepted element and no accepted payload becomes "
            "invalid under a rule that did not exist when it was reviewed. "
            "Every accepted fact "
            "payload, fact key, component key and provenance coordinate "
            "therefore has the same canonical form under both contracts, "
            "which verify_lift proves element by element rather than "
