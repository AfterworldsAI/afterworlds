...
Its **60** findings were reviewed one at a time before anything was baselined. All are
`Hex High Entropy String` and all are deterministic authority content: 45 content-derived `fact_key`
values addressed by `representation.provenance[].target_key[]` (every one recomputed from the loaded
typed objects through `representation.fact_key()`), 3 serialized `fact_key` fields, 2
`proposal_identity`, 2 `semantic_diff_hash`, 3 schema hashes, and the release-binding digests
`authoritative_source_hash`, `bundle_root_hash`, `persisted_corpus_digest`,
`transform_config_hash` and `semantic_policy_hash`. Four were re-derived from first principles as a
cross-check — the policy hash, the schema hash, the proposal identity and the SRD PDF's SHA-256 — and
each matched the value in the file. **No credential is present.**

Entries were generated by `detect-secrets scan --baseline` and spliced into the committed baseline,
so the diff is **+423 / −1**: one results block plus the `generated_at` stamp. The bounded-oracle
fixture, the hazards proposal and the frozen `conditions-1` fixture keep byte-identical entries; no
filter, plugin or detector changed; no directory or glob exclusion was added; no inline allowlist was
written into generated authority.
