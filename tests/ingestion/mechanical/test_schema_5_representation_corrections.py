...
def test_schema_5_stays_pinned_as_the_source_of_a_registered_succession() -> None:
    ...
    This build declares schema 12. Schema 5's pin is unchanged, and what it
    names
    ...
    authority reachable. A drift in this literal would not merely stale a
    canary; it would strand the ``hazards-1`` acceptance, whose artifact
    declares exactly this pair, at the head of a chain that no longer starts
    ...
    onward = SCHEMA_LIFTS[(SCHEMA_6_VERSION, SCHEMA_6_HASH)]
    assert (onward.to_version, onward.to_hash) == (SCHEMA_7_VERSION, SCHEMA_7_HASH)
    onward_again = SCHEMA_LIFTS[(SCHEMA_7_VERSION, SCHEMA_7_HASH)]
    assert (onward_again.to_version, onward_again.to_hash) == (
        SCHEMA_8_VERSION,
        SCHEMA_8_HASH,
    )
    onward_once_more = SCHEMA_LIFTS[(SCHEMA_8_VERSION, SCHEMA_8_HASH)]
    assert (onward_once_more.to_version, onward_once_more.to_hash) == (
        SCHEMA_9_VERSION,
        SCHEMA_9_HASH,
    )
    ...
    onward_yet_again = SCHEMA_LIFTS[(SCHEMA_10_VERSION, SCHEMA_10_HASH)]
    assert (onward_yet_again.to_version, onward_yet_again.to_hash) == (
        SCHEMA_11_VERSION,
        SCHEMA_11_HASH,
    )
    last = SCHEMA_LIFTS[(SCHEMA_11_VERSION, SCHEMA_11_HASH)]
