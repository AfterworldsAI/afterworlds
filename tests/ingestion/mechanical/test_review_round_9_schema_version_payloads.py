...
UNKNOWN_VERSIONS = [
    ...
    # version, then "…-6", then "…-7". Each is declared now, so the probe moves
    # to the one after — the property is that an *unrecognised* version is
...
def test_an_unknown_version_is_refused_even_with_no_components() -> None:
    ...
    with pytest.raises(UnsupportedSchemaVersionError):
        representation_payload(empty, schema_version="5d-representation-schema-8")
...
def test_every_merged_version_states_its_own_key_set() -> None:
    ...
    with pytest.raises(UnsupportedSchemaVersionError):
        representation_payload(
            schema_2_draft(), schema_version="5d-representation-schema-8"
        )
