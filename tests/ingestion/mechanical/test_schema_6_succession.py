...
def test_the_prior_is_not_current_authority_until_it_is_lifted() -> None:
    ...
    assert findings != ()
    assert any(SCHEMA_12_VERSION in f for f in findings), findings
...
def test_the_registered_chain_reaches_current_authority_one_crossing_at_a_time() -> (
    ...
    seven registered steps rather than one: schema 6, 7, 8, 9, 10, 11, then
    12. The
    ...
    assert (records[3].to_version, records[3].to_hash) == (
        SCHEMA_9_VERSION,
        SCHEMA_9_HASH,
    )
    ...
    assert (records[5].to_version, records[5].to_hash) == (
        SCHEMA_11_VERSION,
        SCHEMA_11_HASH,
    )
    assert (records[-1].to_version, records[-1].to_hash) == (
    ...
    assert lifted.oracle.schema_version == SCHEMA_12_VERSION
    assert lifted.oracle.schema_hash == SCHEMA_12_HASH
