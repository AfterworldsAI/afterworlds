...
#: Every recognised contract that predates schema 12. Schema 13 is subtracted
#: beside 12 itself rather than left in: a later succession states every
#: earlier one's meaning, so 13 admits both shapes this module is about and
#: would fail the refusal parametrization for the right reason. Written out
#: rather than derived from version order, for the reason every registry in
#: this area is written out: an unreviewed inheritance is the failure.
